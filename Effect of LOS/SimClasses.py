...
class Entity():
    ...
    def setRealization(self, prob):
        p = SimRNG.Uniform(0, 1, 1)
        if p <= prob:
            self.X = 1
        else:
            self.X = 0
...
class EventNotice():
    ...
    def __init__(self):
        ...
        self.WhichObject = ()
    def __str__(self):
        return f"EventNotice: EventTime is {self.EventTime}, EventType is {self.EventType}"
...
class EventCalendar():
    ...
    def RemoveEvent(self, EventType, TheObject):
#         addedEvent = EventNotice()
#         addedEvent.EventType = EventType
#         addedEvent.WhichObject = TheObject
        for rep in range(0,len(self.ThisCalendar),1):
            if self.ThisCalendar[rep].EventType == EventType and self.ThisCalendar[rep].WhichObject == TheObject:
                    self.ThisCalendar.pop(rep)
...
class FIFOQueue():
    ...
    def RemoveCustomer(self,X):
        self.ThisQueue.remove(X)
        self.WIP.Record(float(self.NumQueue()))
    
    def isInQueue(self,X):
        if X in self.ThisQueue:
            return True
        else:
            return False
